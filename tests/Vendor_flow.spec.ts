...
test.describe.serial("Vendor Login & Flow,@Regression", () => {

  test("Vendor Bid  @TC0014 ,@Smoke, @Regression ", async ({ vendorPage }) => {
  ...
  test("Vendor line item regret @TC0015,@Smoke, @Regression ", async ({ vendorPage }) => {
    await vendorLogin({ page: vendorPage });
   ...
   test("Vendor Side price less than price cap @TC0016,@Smoke, @Regression ", async ({ vendorPage }) => { 
    await vendorLogin({ page: vendorPage });
    ...
    test("Vendor Side price more than price cap @TC0017,@Smoke, @Regression", async ({ vendorPage }) => { 
    await vendorLogin({ page: vendorPage });
    await vendor_bid_tech_priceCap({ page: vendorPage });
  ...
  test("Counter offer Accept Vendor Side @TC0018, @Smoke, @Regression", async ({ vendorPage }) => {
    await vendorLogin({ page: vendorPage });
    await counter_offer_vendor1({ page: vendorPage });
  });

  test("Counter offer Decline Vendor Side @TC0019, @Smoke, @Regression", async ({ vendorPage }) => {
    await vendorLogin({ page: vendorPage });
    await counter_offer_vendor2({ page: vendorPage, counterofferVendorbid:counterofferVendor });
  });

  test("Counter offer Modify Vendor Side @TC0020, @Smoke, @Regression", async ({ vendorPage }) => {
    await vendorLogin({ page: vendorPage });
